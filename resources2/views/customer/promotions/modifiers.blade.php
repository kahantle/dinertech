<form action="{{ route('customer.promotion.menu.add.cart') }}" method="POST" id="addToCart">
    @csrf
    <input type="hidden" value="{{ $item->menu_id }}" name="menuId">
    <input type="hidden" value="{{ $promotionId }}" name="promotionId">
                    ...
                    @foreach ($item->modifierList as $modifierlist)
                       ...
                       @foreach ($modifierlist->modifier_item as $modifierItem)
                            <div class="wd-check-input">
                                <input type="{{ $modifierlist->allow_multiple == 0 ? 'checkbox' : 'radio'}}"
