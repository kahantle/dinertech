...
class OrdersController extends Controller
{
    ...
    public function orderDetail($orderId)
    {
        $uid = Auth::user()->uid;
        $restaurant = RestaurantUser::where('uid', $uid)->first();
        $order = Order::where('restaurant_id', $restaurant->restaurant_id)
        ...
        $data['title'] = 'Order Detail - Dinertech';  
    ...
    public function placeOrder(Request $request)
    {
        ...
        if(empty($request->csv_no_1) || empty($request->csv_no_2) || empty($request->csv_no_3))
        {
            return redirect()->back()->with('error','Invalid card verification number format.');
        }

        $csvNumber = $request->csv_no_1.$request->csv_no_2.$request->csv_no_3;
        $card = Card::where('card_cvv',$csvNumber)->where('card_expire_date',$request->card_exp_date)->where('uid',$uid)->where('restaurant_id',$restaurantId)->first();
        ...
        {
            return redirect()->back()->with('error','Invalid card verification number.');
        }
        ...
        try{
            ...
            $charge = $stripe->charges()->create([
                'card' => $token['id'],
                'currency' => 'USD',
                'amount' => $this->convertNumberFormat($orderDetails['grand_total']),
                'description' => '',
            ]);
            ...
            if($charge['status'] == 'succeeded') 
            {
                ...
                // $order->cart_charge = $this->convertNumberFormat($orderDetails['cart_charge']);
                // $order->delivery_charge = '0.00';
                // $order->discount_charge = $this->convertNumberFormat($orderDetails['discount_charge']);
                // $order->is_feature = ($orderDetails['order_status'] == 1) ? 1 : 0;
                // $order->order_status = ($orderDetails['order_status'] == 1) ? 1 : 0;
                ...
                // else
                // {
                //     // $order->order_date = date("Y-m-d",strtotime($orderDetails['orderDate']));
                //     // $order->order_time = date("h:m:A",strtotime($orderDetails['orderTime']));
                //     $order->order_date = date('Y-m-d');
                //     $order->order_time = date('h:m:A');
                ...
                // $order->isPickUp = 0;
                // $order->comments = $orderDetails['instruction'];
                ...
                //         {
                //             if($menuItemData->menu_id == $menuItem->menu_id)
                //             {
                ...
                //     $data['title'] = 'Success Order - Dinertech';
