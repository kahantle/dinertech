            ...
            <div class="dash-second">
                <div class="container-fluid">
                    @if ($orders)
                            ...
                            <div class="recent-order-blog">
                                <div class="row">
                                    <div class="col-md-12 d-flex justify-content-between">
                                        <div class="order-recent-new {{ $order->order_progress_status == Config::get('constants.ORDER_STATUS.ORDER_DUE') ? 'order-recent-red' : 'order-new-green' }}"
                                            id="change-color-{{ $order->order_id }}">
                                        ...
                                        <div class="order-recent-pickup">
                                            ...
                                            @if ($order->order_status == '')
                                                ...
                                                <a href="javaScript:void(0);"
                                                    ...
                                                    class="action" data-value="Cancel">
                                                    <button href="javaScript:void(0);" data-toggle="tooltip"
                                                        title="Decline Order!" class="action btn-danger"
                                                        data-value="Cancel">
                                            ...
                                            @else
                                                @if ($order->order_progress_status != Config::get('constants.ORDER_STATUS.ORDER_DUE'))
                                                    <div class="order-timer order-time-{{ $order->order_id }}"
                                                        data-pickup="{{ date_format(date_create($order->pickup_time), 'Y-m-d H:i:s') }}"
                                                        data-pickup-minutes="{{ $order->pickup_minutes }}"
                                                        data-orderId="{{ $order->order_id }}"></div>
                                                @else
                                            ...
                                            @else
                                                <a href="javaScript:void(0);"
                                                    ...
                                                    data-toggle="tooltip" title="Print Order Receipt!" class="">
                                                    <button class="disabled-order-print">
                        ...
                        @endforeach
                        <div class="w-100 pagination-links"> {{ $orders->links('vendor.pagination.bootstrap-4') }}</div>
                    @else
